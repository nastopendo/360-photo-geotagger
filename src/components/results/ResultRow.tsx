...
export function ResultRow({ result, photo360, referencePhoto, isSelected, onSelect }: ResultRowProps) {
  ...
  return (
    <tr
      className={`border-b border-gray-100 text-xs transition-colors ${editing ? 'bg-yellow-50' : isSelected ? 'bg-blue-50 cursor-pointer hover:bg-blue-50' : 'cursor-pointer hover:bg-blue-50'}`}
      onClick={editing ? undefined : onSelect}
    >
      <td className="max-w-[140px] truncate py-2 pl-3 pr-2 font-medium text-gray-800" title={photo360.name}>
        {photo360.name}
      </td>
        ...
        {editing ? (
          <form className="flex items-center gap-1" onSubmit={commitEdit}>
            ...
            <input
              ...
              onClick={(e) => e.stopPropagation()}
            />
            <button type="submit" className="text-green-600 hover:text-green-700 font-bold" onClick={(e) => e.stopPropagation()}>✓</button>
            <button type="button" className="text-gray-400 hover:text-gray-600 font-bold" onClick={(e) => { e.stopPropagation(); cancelEdit() }}>✗</button>
          </form>
        ) : (
